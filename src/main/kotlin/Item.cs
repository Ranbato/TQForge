...
namespace TQ_weaponsmith
{
  public class Item
  {
    private static Random random = Item.InitializeRandom();
    ...
    {
      Item.SetDefaultStrings();
      this.baseItemId = id.ToUpperInvariant();
      this.recordType = recordType.ToUpperInvariant();
      this.itemClassification = classification.ToUpperInvariant();
    ...
    private static string CheckExtension(string itemId)
    {
      if (itemId == null)
        return (string) null;
      return itemId.Length < 4 || Path.GetExtension(itemId).ToUpperInvariant().Equals(".DBR") ? itemId : itemId + ".dbr";
    }

    private bool checkSubstring(string substring) => this.hasSubstring(this.BaseItemId, substring) || this.hasSubstring(this.prefixID, substring) || this.hasSubstring(this.suffixID, substring);
    ...
    public static string ClipColorTag(string text)
    {
      ...
      if (text.Contains("{^"))
      {
        ...
        if (length != -1)
        {
          ...
          text = text.Substring(0, length) + text.Substring(num + 1);
        }
      }
      return text;
    }
      ...
      List<string> results)
    {
      ...
      foreach (Variable attribute in attributeList)
      {
        if (attribute != null)
        {
          ...
          switch (itemAttributesData8.Variable.ToUpperInvariant())
          {
            case "CHANCE":
      ...
      foreach (Variable attribute in attributeList)
      {
        if (attribute != null)
        {
          ...
          string upperInvariant1 = itemAttributesData9.Variable.ToUpperInvariant();
          if ((str1 == null || upperInvariant1 != "MIN" && upperInvariant1 != "MAX" && upperInvariant1 != "DRAINMIN" && itemAttributesData9.Variable != "DRAINMAX") && (str2 == null || upperInvariant1 != "DURATIONMIN" && upperInvariant1 != "DURATIONMAX") && (str4 == null || upperInvariant1 != "CHANCE") && (str8 == null || upperInvariant1 != "MODIFIER") && (str9 == null || upperInvariant1 != "DURATIONMODIFIER") && (str10 == null || upperInvariant1 != "MODIFIERCHANCE") && (str3 == null || upperInvariant1 != "DAMAGERATIO") && upperInvariant1 != "GLOBAL" && !(upperInvariant1 == "XOR" & isGlobal))
          {
            ...
            string upperInvariant2 = itemAttributesData9.FullAttribute.ToUpperInvariant();
    ...
    public void Encode(OutputStream writer)
    {
      ...
      if (this.sackType != SackType.Sack && this.sackType != SackType.Player)
      {
        ...
        writer.Write(Convert.ToSingle((object) num1, (IFormatProvider) CultureInfo.InvariantCulture));
        TQData.writeCString(writer, "yOffset");
        writer.Write(Convert.ToSingle((object) num2, (IFormatProvider) CultureInfo.InvariantCulture));
      }
      else
      {
        while (true)
    ...
    {
      string upperInvariant = key.ToUpperInvariant();
      ...
      for (int index = 0; index < variable.NumberOfValues; ++index)
      {
        switch (variable.DataType)
        {
          ...
          case VariableDataType.StringVar:
            if ((allowStrings || variable.Name.ToUpperInvariant().Equals("CHARACTERBASEATTACKSPEEDTAG") || variable.Name.ToUpperInvariant().Equals("ITEMSKILLNAME") || variable.Name.ToUpperInvariant().Equals("SKILLNAME") || variable.Name.ToUpperInvariant().Equals("PETBONUSNAME") || ItemAttributes.IsReagent(variable.Name)) && variable.GetString(index).Length > 0)
              return false;
            break;
    ...
    public static int GenerateSeed() => Item.random.Next((int) short.MaxValue);
    ...
    public string GetAttributes(bool filtering)
    {
      if (this.attributesString != null)
      {
        ...
        if (this.baseItemInfo != null)
        {
          ...
          {
            string upperInvariant = this.baseItemInfo.GetString("artifactClassification").ToUpperInvariant();
    ...
    public Color GetColorTag(string text)
    {
      if (!string.IsNullOrEmpty(text))
      {
        ...
        if (text.StartsWith("^", StringComparison.OrdinalIgnoreCase))
        {
          str = text.Substring(1, 1).ToUpperInvariant();
        }
        else
        {
          int num = text.IndexOf("{^", StringComparison.OrdinalIgnoreCase);
          str = num != -1 ? text.Substring(num + 2, 1).ToUpperInvariant() : (string) null;
        }
        if (string.IsNullOrEmpty(str))
      ...
      Info itemInfo)
    {
      ...
      foreach (Variable variable2 in recordFromFile2)
      {
        if (string.Compare(variable2.Name, 0, requirementEquationPrefix, 0, requirementEquationPrefix.Length, StringComparison.OrdinalIgnoreCase) <= 0)
        {
          ...
          string key = str2.Replace(str2[0], char.ToUpperInvariant(str2[0]));
          if (key.Equals("Level"))
    ...
    private bool hasSubstring(string fullstring, string substring) => fullstring != null && fullstring.ToUpperInvariant().IndexOf(substring, StringComparison.OrdinalIgnoreCase) >= 0;

    private static Random InitializeRandom() => new Random();
    ...
    public void Parse(InputStream reader)
    {
      try
      {
        ...
        if (TQData.readCString(reader).ToUpperInvariant().Equals("relicName2".ToUpperInvariant()))
        {
        ...
        if (this.sackType == SackType.Stash)
        {
          ...
          this.PositionX = Convert.ToInt32((object) reader.ReadSingle(), (IFormatProvider) CultureInfo.InvariantCulture);
          TQData.validateNextString("yOffset", reader);
          this.PositionY = Convert.ToInt32((object) reader.ReadSingle(), (IFormatProvider) CultureInfo.InvariantCulture);
        }
    ...
    public static int WeaponSlotIndicator => -3;
    ...
    {
      get => this.baseItemId;
      private set => this.baseItemId = value;
    }
    ...
    public bool IsImmortalThrone => this.checkSubstring("XPACK\\");

    public bool IsRagnarok => this.checkSubstring("XPACK2\\");

    public bool IsAnyXPack => this.IsImmortalThrone || this.IsRagnarok;
    ...
    public bool IsScroll
    {
      get
      {
        ...
        return this.IsAnyXPack && this.BaseItemId.ToUpperInvariant().IndexOf("\\SCROLLS\\", StringComparison.OrdinalIgnoreCase) >= 0;
      }
    }

    public bool IsParchment => this.IsAnyXPack && this.BaseItemId.ToUpperInvariant().IndexOf("\\PARCHMENTS\\", StringComparison.OrdinalIgnoreCase) >= 0;
    ...
    public bool IsFormulae
    {
      get
      {
        ...
        return this.IsAnyXPack && this.BaseItemId.ToUpperInvariant().IndexOf("\\ARCANEFORMULAE\\", StringComparison.OrdinalIgnoreCase) >= 0;
      }
    }
    ...
    public bool IsArtifact
    {
      get
      {
        ...
        return this.IsAnyXPack && !this.IsFormulae && this.BaseItemId.ToUpperInvariant().IndexOf("\\ARTIFACTS\\", StringComparison.OrdinalIgnoreCase) >= 0;
      }
    }

    public bool IsShield => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONARMOR_SHIELD");

    public bool IsArmor => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().StartsWith("ARMORPROTECTIVE", StringComparison.OrdinalIgnoreCase);

    public bool IsHelm => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_HEAD");

    public bool IsBracer => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_FOREARM");

    public bool IsTorsoArmor => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_UPPERBODY");

    public bool IsGreave => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_LOWERBODY");

    public bool IsRing => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORJEWELRY_RING");

    public bool IsAmulet => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORJEWELRY_AMULET");

    public bool IsWeapon => this.baseItemInfo != null && !this.IsShield && this.baseItemInfo.ItemClass.ToUpperInvariant().StartsWith("WEAPON", StringComparison.OrdinalIgnoreCase);
    ...
    public bool Is2HWeapon
    {
      get
      {
        if (this.baseItemInfo == null)
          return false;
        return this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONHUNTING_BOW") || this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONMAGICAL_STAFF");
      }
    }

    public bool IsWeaponShield => this.baseItemInfo != null && this.baseItemInfo.ItemClass.ToUpperInvariant().StartsWith("WEAPON", StringComparison.OrdinalIgnoreCase);
    ...
    public ItemStyle ItemStyle
    {
      get
      {
        ...
        if (this.suffixInfo != null && this.suffixInfo.ItemClassification.ToUpperInvariant().Equals("RARE") || this.prefixInfo != null && this.prefixInfo.ItemClassification.ToUpperInvariant().Equals("RARE"))
          return ItemStyle.Rare;
    ...
    public bool HasRelic => !this.IsRelic && this.relicID.Length > 0;
    ...
    public bool IsPotion
    {
      get
      {
        ...
        return this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ONESHOT_POTIONHEALTH") || this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ONESHOT_POTIONMANA");
      }
    }
    ...
    public bool IsCharm
    {
      get
      {
        ...
        return !this.IsImmortalThrone ? this.BaseItemId.ToUpperInvariant().IndexOf("ANIMALRELICS", StringComparison.OrdinalIgnoreCase) != -1 : this.BaseItemId.ToUpperInvariant().IndexOf("\\CHARMS\\", StringComparison.OrdinalIgnoreCase) != -1;
      }
    }
    ...
    public bool IsRelic
    {
      ...
      {
        if (this.baseItemInfo != null)
          return this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMRELIC") || this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMCHARM");
        if (!this.IsAnyXPack)
          return this.BaseItemId.ToUpperInvariant().IndexOf("RELICS", StringComparison.OrdinalIgnoreCase) != -1;
        return this.BaseItemId.ToUpperInvariant().IndexOf("\\RELICS\\", StringComparison.OrdinalIgnoreCase) != -1 || this.BaseItemId.ToUpperInvariant().IndexOf("\\CHARMS\\", StringComparison.OrdinalIgnoreCase) != -1;
      }
    }
    ...
    public bool IsRelicComplete
    {
      get
      {
        if (this.baseItemInfo == null)
          return false;
        return this.Var1 >= this.baseItemInfo.CompletedRelicLevel || this.baseItemInfo.CompletedRelicLevel == 1;
      }
    }
      ...
      get
      {
        if (this.baseItemInfo != null)
        {
          ...
          else if (this.IsArtifact)
          {
            ...
            if (str.ToUpperInvariant() == "E_GA_SANDOFKRONOS")
              str = str.Insert(9, "s");
    ...
    public int ItemGroup
    {
      get
      {
        ...
        {
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ONESHOT_POTIONMANA"))
            return 1;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORJEWELRY_AMULET"))
            return 2;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORJEWELRY_RING"))
            return 3;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMCHARM"))
            return 4;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMRELIC"))
            return 5;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMARTIFACTFORMULA"))
            return 6;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ITEMARTIFACT"))
            return 7;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ONESHOT_SCROLL"))
            return 8;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_LOWERBODY"))
            return 9;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_FOREARM"))
            return 10;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_HEAD"))
            return 11;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("ARMORPROTECTIVE_UPPERBODY"))
            return 12;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONARMOR_SHIELD"))
            return 13;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONMELEE_AXE"))
            return 14;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONMELEE_MACE"))
            return 15;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONMELEE_SWORD"))
            return 16;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONHUNTING_BOW"))
            return 17;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONMAGICAL_STAFF"))
            return 18;
          if (this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONHUNTING_SPEAR"))
            return 19;
          itemGroup = !this.baseItemInfo.ItemClass.ToUpperInvariant().Equals("WEAPONHUNTING_RANGEDONEHAND") ? 0 : 20;
        }
        return itemGroup;
      }
    }
