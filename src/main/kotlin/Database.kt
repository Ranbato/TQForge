...
//
    private fun FigureDBFileToUse(rootFolder:String):String?
    {
      ...
      val files =  Files.walk(Path.of(rootFolder)).filter { item -> item.name.matches(Regex.fromLiteral("Text_??.arc")) }.findFirst()
...
//    });
    ...
    public fun LoadArcFileData(
       ...
       alternateFolder:Boolean):ByteArray
    {
    ...
    private fun LoadARZFile()
    {
      ...
      try
      {
        ...
        this.ArzFileAE = this.LoadARZFile(Path.of(installLocation, "Database/database.arz").toString(), "Anniversary Edition");
      }
      catch ( ex:Exception)
      {
    ...
    private fun ParseTextDB( databaseFile:String,  filename:String)
    {
      ...
      else
      {
        BufferedReader(InputStreamReader(ByteArrayInputStream(buffer))).use { streamReader ->
          val ch = '='
          var str1:String? = ""
          while (run{str1 = streamReader.readLine()
          return@run str1} != null)
          {
            val str2 = str1!!.trim();
